        ...
        public bool ConsultaSAP()
        {
            ...
            if (ped != null)
            {
                ...
                if (this.CN47N.Count > 0)
                {
                    ...
                    if (avanco_sap[ZGPP_GET_AVANCO_TABS.PECAS].Count > 0)
                    {
                        this.ZPMP = avanco_sap[ZGPP_GET_AVANCO_TABS.PECAS].Select(x => new sapgui.ZPMP(x, true)).ToList();
                        this.ZPP0100 = avanco_sap[ZGPP_GET_AVANCO_TABS.CARGAS].Select(x => new sapgui.ZPP0100(x, true)).ToList();
